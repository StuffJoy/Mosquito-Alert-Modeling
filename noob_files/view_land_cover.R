# Install (once) and load
install.packages("terra")       # skip if already installed
library(terra)

# 1) Read the GeoTIFF
wc <- rast("data/Bangladesh_WorldCover_2021.tif")   # path to your file
plot(wc)
...
# 4) Quick look
plot(wc, col = cols, type = "classes", plg = list(title = "ESA WorldCover 2021"))

# 5) Basic counts by class (pixels)
freq(wc)  # table of code -> pixel count present in your raster
...
ha_per_cell <- prod(res(wc)) * 111320 * 111320 / 10000
# ^ ONLY if wc is in degrees; but your export used EPSG:4326, so this would be rough.
# A better way: reproject to an equal-area CRS first, then sum pixelArea.
wc_eq <- project(wc, "EPSG:6933")  # World Cylindrical Equal Area
cell_ha <- cellSize(wc_eq, unit = "ha")
area_tab <- as.data.frame.zoo(zonal(cell_ha, wc_eq, "sum"))
